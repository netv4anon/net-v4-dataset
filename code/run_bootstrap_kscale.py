...
def pred_EventAwareLast(x, x_ev_in, shock_prof):
    """
    Non-oracle event-aware heuristic using only the observed input window.

    Applies the learned shock profile only when the last observed input step is
    already EV-active, approximating an ongoing intervention at forecast origin.
    """
